...
void conjunto_uniao(conjunto_t *a, conjunto_t *b, conjunto_t *c){
    if(a != NULL && b != NULL && c != NULL){
        ...
        if((c -> capacidade) > 0){
            ...
            if((c -> numero) < (c -> capacidade)){
                (c -> vetor) = (int*) realloc ((c -> vetor), sizeof(int)*(c -> numero));
            }
        }
    }
}
...
void conjunto_diferenca(conjunto_t *a, conjunto_t *b, conjunto_t *c){
    if(a != NULL && b != NULL && c != NULL){
        ...
        if(((b -> numero) > 0) || (a -> numero) > 0){
            ...
            if((c -> numero) < (c -> capacidade) && (c -> numero) > 0){
                (c -> vetor) = (int*) realloc ((c -> vetor), sizeof(int)*(c -> numero));
            }
        }
    }
...
int conjunto_insere_elemento(elem_t x, conjunto_t *a){
    if (a != NULL){
        ...
        if((a -> numero) == (a -> capacidade)){
            ( a-> vetor) = (int*) realloc ((a -> vetor), sizeof(int) * ((a -> numero) + 1));
            (a -> vetor[(a -> numero)]) = x;
            (a -> numero) += 1;
            (a -> capacidade) += 1;
            return 1;
    ...
    if (a != NULL){
        ...
        if( (a -> numero) > 0){
            if( (a -> numero) > 1){
                ...
                for(j = 0; j < (a -> numero); j++){
                    if( j != ((a -> numero) -1) && j != 0 && a -> vetor[j] == a -> vetor[j - 1]){
                         a -> vetor[j] = a -> vetor[j + 1];
                    }
                } (a -> numero) -= 1;
                if((a -> numero) < (a -> capacidade)){
                    (a-> vetor) = (int*) realloc ((a -> vetor), sizeof(int) * (a -> numero));
                    (a -> capacidade) = (a -> numero);
                }
            }
            else if ((a -> numero) == 1){
                if((a -> vetor[0]) == x){
